...
--
-- If the background gas velocity vector v0=(gas_vx,gas_vy,gas_vz) is
-- non-zero, the above analysis will still work if you shift the
-- velocity frame of reference.  This can be done simply by replacing
-- ion_vx_mm with (ion_vx_mm - gas_vx) and likewise for Y and Z.
-- You can also confirm this works by changing Equation 1 to
-- "a_vis = -linear_damping * (v-v0)" and following through the math to obtain
-- the same result.
--
-- HISTORY:
-- 2010-07 - added gas_vx/y/vz variables
...
-- background gas velocity (mm/microsecond) in X, Y, and Z directions.
-- Set these to 0 for stationary background gas.
-- You can also make these a function of position
-- (ion_px_mm, ion_py_mm, ion_pz_mm) by defining equations for them at
-- the top of the accel_adjust segment, prior to their use.
adjustable gas_vx = 0
adjustable gas_vy = 0
adjustable gas_vz = 0
...
function segment.accel_adjust()
    ...
    ion_ax_mm = factor * (ion_ax_mm - linear_damping * (ion_vx_mm - gas_vx))
    ion_ay_mm = factor * (ion_ay_mm - linear_damping * (ion_vy_mm - gas_vy))
    ion_az_mm = factor * (ion_az_mm - linear_damping * (ion_vz_mm - gas_vz))
end
