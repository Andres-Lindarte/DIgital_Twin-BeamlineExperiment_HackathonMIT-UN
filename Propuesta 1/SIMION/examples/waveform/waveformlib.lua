...
-- By default, segments are installed into SIMION's segment table.
-- Defined by user.
...
function WAVE.fast_adjust()
  local t = ion_time_of_flight
  for i,wave in pairs(WAVE.waves) do
    local v = get_potential(wave, t)
    -- print('DEBUG:',t, v)
...
function WAVE.other_actions()
  ...
  if abs(ion_time_of_flight - last_tof) > period then
    ...
    for i,wave in pairs(WAVE.waves) do
      local n = get_piece(wave, ion_time_of_flight - ion_time_step * 0.5)
