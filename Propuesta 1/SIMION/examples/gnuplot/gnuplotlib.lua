...
function GNUPLOT.open()
  ...
  local fh
  if osname and not (osname:match'MS%-Windows' or osname:match'CYGWIN') then
...
function GNUPLOT.plot(t)
  ...
  local plot = setmetatable({fh=fh}, {__index = Plot})
  ...
  for i,dataset in ipairs(datasets) do
    ...
    local ncols = #(dataset.header or dataset[1])
    for c=2,ncols do
      fh:write(('%s"tmp%d.dat" using 1:%d title "%s" '):format(
                (iseries==1 and '' or ', '), i, c, header[c] or ''))
      iseries = iseries + 1
    end
  end
  fh:write('with ', CHART_TYPES[chart_type], ' ')
  fh:write('\n')
  fh:flush()
...
function GNUPLOT.plot_surface(t)
  ...
  local function doplot()
    ...
    for yi=1,#ys do
      for xi=1,#xs do
        fh:write(("%g "):format((t[xi] or {})[yi] or 0), ((xi==#xs or #ys==1) and '\n' or ''))
      end
    end
    fh:write(is2d and 'e\ne\n' or 'e\n')
    fh:flush()
