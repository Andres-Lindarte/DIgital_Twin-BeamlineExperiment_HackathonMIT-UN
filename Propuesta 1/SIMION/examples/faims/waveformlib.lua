--[[
 ...
 v2012-01-01, D.Manura, PRE-RELEASE version
...
-- Default time-step to use.  Microseconds.
-- Overrides T.Qual if set.
WAVE.recommended_timestep_ = nil
...
-- (e.g. to Excel (or gnuplot) and/or log window).
...
function WAVE.segment.tstep_adjust()
  if WAVE.recommended_timestep_ then  -- default value
    ion_time_step = WAVE.recommended_timestep_
  end
...
function WAVE.install_segment(newsegment)
...
function WAVE.install(t)
  ...
  WAVE.install_segment(WAVE.segment)
end


-- Sets waveform.  This may be called after WAVE.install to change
-- waveform.
function WAVE.set_waveforms(waveforms)
  WAVE.waveforms_ = waveforms
end

-- Sets recommended time-step size in microseconds.
-- If not nil, this overrides T.Qual.
-- Time-step sizes may still be reduced at transitions though.
function WAVE.set_recommended_timestep(step)
  WAVE.recommended_timestep_ = step
end
