...
-- 1 = plot gas flow; 0 = no plot
adjustable flow_plot = 1
...
function SDS.init()
  ...
  if poiseuille_flow ~= 0 then
    ...
    FLOW.define_poiseuille_cylindrical_transverse {
      SDS = SDS,
      ...
      vt_m_psec = SDS_vx_m_per_sec, -- max velocity (m/s) in center
      mu_pa_s = mu_pa_s  -- dynamic viscosity (Pa s)
    }
  else -- constant flow
    local FLOW = simion.import "flowlib.lua"
    FLOW.define_cylindrical_transverse {
      SDS = SDS,
      d_mm = 0.5,  -- Distance between plates (mm)
      y0_mm = -1.25,  -- Y center of cylinders (mm)
      z0_mm = 0,      -- Z center of cylinders (mm)
  ...
  if flow_plot ~= 0 then
    ...
    CON.plot{func=SDS.velocity,  npoints=60, x=2.5, mark=true}
    --CON.plot{func=SDS.pressure,  npoints=60, x=2.5, mark=true}
  end
end
