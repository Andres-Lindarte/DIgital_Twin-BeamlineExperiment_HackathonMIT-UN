...
local M = {}
M._VERSION = '8.1.1.30.20130222-FAIMS'
...
--
-- Background gas velocity vector at ion's local position.
--
local ions_local_vx_mm_per_usec = {}
local ions_local_vy_mm_per_usec = {}
local ions_local_vz_mm_per_usec = {}
-- END FAIMS
...
local ions_local_dvoltsx_Vmm = {}   -- local field (V/mm) at end of time step
local ions_local_dvoltsy_Vmm = {}   --   keyed by ion number.
local ions_local_dvoltsz_Vmm = {}
-- END FAIMS
...
--## Quick RF Cycle Feature.
-- This is an optional feature intended to greatly improve calculation speed.
-- It traces a particle over one or two (SDS_quick_normal_cycles) number of
-- RF cycles in the normal way (except without diffusion), measures the net
-- displacement, and then, rather than tracing the subsequent SDS_quick_cycles
-- number of RF cycles, it simply extrapolates the displacement.
-- Finally, it applies diffusion (if enabled) expected for both the normal and
-- extrapolated cycles.
-- See the README.html file for details on this feature.

-- Number of RF cycles to extrapolate after each set of normally traced
-- RF cycles.
--   0 (default) : disable quick RF cycle feature.
--   integer > 0 : number of cycles to extrapolate
--   -1 : automatically sets the number of cycles to extrapolate
--       in such a way that the net displacement of the particle is no
--       more than approximately one grid unit.
--         Note: a possible improvement is to add an additional parameter
--         (e.g. SDS_quick_step) that limits the extrapolation to
--         a given distance.  That will negate the need for this -1 option.
-- See the README.html file for details on this feature.
adjustable SDS_quick_cycles = 0

-- Number of RF cycles to trace normally between skipped RF cycles.
-- This must be be 1 or 2.
-- If 1, then the extrapolation is linear, advancing the particle
--   along a linear path during the extrapolated cycles.
-- If 2, then the extrapolation is second-order (higher accuracy), which
--   extrapolates along a curved path derived from the positions and
--   angle between the two normally traced cycles.
-- 2 is recommended.  Although it doubles the number of normally traced
--   RF cycles (which slows the calculation), it can improve accuracy
--   under a given number of extrapolated RF cycles (SDS_quick_cycles),
--   thereby allowing SDS_quick_cycles to be increased (speeds calculation).
-- 1 may be preferred if the averaged movement of the particle is expected
--   to be linear, such as in a uniform field (e.g. parallel plates).
-- This is only used if SDS_quick_cycles is non-zero.
-- See the README.html file for details on this feature.
adjustable SDS_quick_normal_cycles = 2

-- Time period of RF cycle (in usec).
...
local function update_local_parameters()
  ...
  if (pres_obj or temp_obj
     or ions_alphabeta[ion_number]  -- FAIMS
     ) and local_pressure_torr ~= 0
  then
...
-- BEGIN FAIMS
-- variables used by "quick RF cycle" feature, keyed by ion_number.
local last_time = {}
local last_x = {}
local last_y = {}
local last_z = {}
...
--[[
 Updates mass of current ion, as well as mass dependent variables.
...
function M.update_ion_mass(new_mass)
  ...
  local mfpmass, vmass, ko,
        alpha, beta -- FAIMS
    = massdata_for_mass(massdata, new_mass)
  ...
  local logmrratio = log10(new_mass / SDS_collision_gas_mass_amu)
...
function M.segment.accel_adjust()
  ...
  -- BEGIN FAIMS
  -- Note: accel_adjust is called over multiple points in the time
  -- step, but we only want to measure local parameters at the last point
  -- (i.e. start of next time step).
  ...
  if SDS_faims_mode ~= 0 and ion_time_step < (time_e[ion_number] or 0) then
    return -- don't recalculate parameters for previous points.
  end
  ...
  if is_local_params
     or SDS_faims_mode ~= 0 or ions_alphabeta[ion_number] -- FAIMS
  then
    update_local_parameters()
    
    -- BEGIN FAIMS
    -- Save parameters needed in other_actions segment.
    -- Key these by ion_number in case Grouped flying is used.
    ions_local_dvoltsx_Vmm[ion_number] = local_dvoltsx_Vmm
    ions_local_dvoltsy_Vmm[ion_number] = local_dvoltsy_Vmm
    ions_local_dvoltsz_Vmm[ion_number] = local_dvoltsz_Vmm
    ions_local_vx_mm_per_usec[ion_number] = local_vx_mm_per_usec
    ions_local_vy_mm_per_usec[ion_number] = local_vy_mm_per_usec
    ions_local_vz_mm_per_usec[ion_number] = local_vz_mm_per_usec
    -- END FAIMS
  end
...
function M.segment.other_actions()
  ...
  -- Disable SDS effects when pressure zero.
  if local_pressure_torr == 0 then
    return
  end
  ...
  if SDS_faims_mode ~= 0 then
    -- Note: this section uses the electric field and mobility at the end of
    -- this time-step.  Therefore, in a "voltage transition time-step", this
    -- field is representative of the next regular time step.  Also,
    -- the electric field used here actually is instead a total acceleration
    -- per charge (where acceleration is derived not just from the
    -- electrostatic PA but all sources--electric and magnetic fields,
    -- including charge repulsion electric fields from particles.
    -- (In constract, ion_dvolts[xyz]_gu represents the PA electric field
    -- at the beginning of the time step.)

    -- Compute ion mobility constant from ion's local damping constant.
    local damping = ions_local_damping[ion_number]
    local emu = ELEMENTARY_CHARGE/AMU_TO_KG    -- (C kg-1 u)
    ...
    local ko_conversion = ko * (1000 * 1E-4 * 1E-3) * (ion_charge < 0 and -1 or 1)
    ion_vx_mm = ions_local_vx_mm_per_usec[ion_number] - ions_local_dvoltsx_Vmm[ion_number] * ko_conversion
    ion_vy_mm = ions_local_vy_mm_per_usec[ion_number] - ions_local_dvoltsy_Vmm[ion_number] * ko_conversion
    ion_vz_mm = ions_local_vz_mm_per_usec[ion_number] - ions_local_dvoltsz_Vmm[ion_number] * ko_conversion
  end
  ...
  if SDS_quick_cycles ~= 0 then
    ...
    local dtp = dt / wave_period
    local TOL = ion_time_step / 10
    if abs(dtp - 1) < TOL then -- i.e. very close to 1 RF cycle
      if SDS_quick_normal_cycles == 1 then
        --## Linearly extrapolate if doing one normal RF cycle.

        -- Compute displacement over one RF cycle.
        local dx = ion_px_mm - last_x[ion_number]
        local dy = ion_py_mm - last_y[ion_number]
        local dz = ion_pz_mm - last_z[ion_number]
        --
        -- Determine number of extrapolated RF cycles.
        local NQUICK
        ...
        -- Simulate quick RF cycles by linearly extrapolating ion position.
        ...
        delta_time = dt * (SDS_quick_normal_cycles + NQUICK)
      else
        -- Record data at end of first normal RF cycle.
        next_time[ion_number] = ion_time_of_flight
        next_x[ion_number] = ion_px_mm
        next_y[ion_number] = ion_py_mm
        next_z[ion_number] = ion_pz_mm
      end

    elseif SDS_quick_normal_cycles == 2 and abs(dtp - 2) < TOL
           and next_time[ion_number] then
                                  -- i.e. very close to second RF cycle

      -- Compute displacement over one RF cycle.
      local dx = ion_px_mm - next_x[ion_number]
      local dy = ion_py_mm - next_y[ion_number]
      local dz = ion_pz_mm - next_z[ion_number]
      --
      -- Determine number of extrapolated RF cycles.
      local NQUICK
      if SDS_quick_cycles == -1 then -- automatically choose
      ...
      -- Simulate quick RF cycles by extrapolating ion position.
      -- Using Lagrange interpolation (n = 3).
      -- http://mathworld.wolfram.com/LagrangeInterpolatingPolynomial.html
      local t1 = last_time[ion_number]
      local t2 = next_time[ion_number]
      local t3 = ion_time_of_flight
      local t  = ion_time_of_flight + dt/2 * NQUICK
      local x1 = last_x[ion_number]
      local y1 = last_y[ion_number]
      local z1 = last_z[ion_number]
      local x2 = next_x[ion_number]
      local y2 = next_y[ion_number]
      local z2 = next_z[ion_number]
      local x3 = ion_px_mm
      local y3 = ion_py_mm
      local z3 = ion_pz_mm
      local k1 = (t - t2)*(t - t3)/((t1 - t2)*(t1 - t3))
      local k2 = (t - t1)*(t - t3)/((t2 - t1)*(t2 - t3))
      local k3 = (t - t1)*(t - t2)/((t3 - t1)*(t3 - t2))
      local x = k1*x1 + k2*x2 + k3*x3
      local y = k1*y1 + k2*y2 + k3*y3
      local z = k1*z1 + k2*z2 + k3*z3
      --[[ -- example of linear interpolation, only for comparison
      local x = x3 + (x3-x1)/(t3-t1) * (dt/2 * NQUICK)
      local y = y3 + (y3-y1)/(t3-t1) * (dt/2 * NQUICK)
      local z = z3 + (z3-z1)/(t3-t1) * (dt/2 * NQUICK)
      --]]
      ...
      last_time[ion_number] = nil -- clear normal RF cycle
      next_time[ion_number] = nil
      --mark()

      -- Now apply diffusion over delta_time (normal cycles + quick RF cycles)
      allow_diffusion = true
      delta_time = dt/SDS_quick_normal_cycles * (SDS_quick_normal_cycles + NQUICK)
      
    elseif dtp > SDS_quick_normal_cycles then
           -- failed to accurately measure normal RF cycles
