...
function M.define_poiseuille_planar(t)
  ...
  SDS.velocity = function(x,y,z)
    ...
    local vx = vx_m_psec * math.max(0, 1 - r*r)    -- (m/sec)
    return vx, 0, 0
  end
...
--[[
 ...
 along axial direction.  Cylinder axis is in X direction.
 Approximates gap as infinite parallel plates, which may not
 be valid if ratio of cylinder radii is large.

 Note: This is similar to the planar version in that
 ...
 `t` is a table containing these fields:
   ...
   vx_m_psec -- maximum gas velocity, in center of plates.
             --   This is (3/2) the average velocity.
             --   Positive gas velocity is in +X direction.
   mu_pa_s   -- dynamic viscosity (Pa s)
--]]
function M.define_poiseuille_cylindrical(t)
  ...
  SDS.velocity = function(x,y,z)
    ...
    return vx * math.max(0, 1 - r*r), 0, 0
  end
...
--[[
 Defines Poiseuille [4] gas flow between infinite concentric cylinders,
 in circular direction around (traverse to) axis.
 Cylinder axis is in X direction.
 Approximates gap as infinite parallel plates, which may not
 be valid if ratio of cylinder radii is large.
 ...
 `t` is a table containing these fields:
   ...
   SDS       -- SDS object flow is to be applied to.
   d_mm      -- Distance between plates (mm)
   y0_mm     -- Y center of cylinders (mm)
   z0_mm     -- Z center of cylinders (mm)
   p0_torr   -- Pressure at x=x0_mm (torr)
   vt_m_psec -- maximum gas velocity, in center of plates.
             --   This is (3/2) the average velocity.
             --   Positive gas velocity in CCW direction
             --   looking down positive X axis.
   mu_pa_s   -- dynamic viscosity (Pa s)
  
 The pressure decreases in the counter-clockwise direction, starting
 at maximum Y.  There is a discontinuity here which would be
 observed if a particle loops around (which isn't realistic).
--]]
function M.define_poiseuille_cylindrical_transverse(t)
  t = begin_check(t)
  local SDS       = t.SDS
  local d_mm      = t.d_mm
  local y0_mm     = t.y0_mm
  ...
  SDS.velocity = function(x,y,z)
    -- Parabolic profile.
    local rc = sqrt((y-y0_mm)^2 + (z-z0_mm)^2)
    local r = (rc - R0) / (0.5*d_mm)  -- (unitless, scaled to +-1 range)
    local vt = vt_m_psec  -- (m/s)
    local f = math.max(1 - r*r, 0) * vt / rc
    local uy = -(z-z0_mm) * f
    local uz =  (y-y0_mm) * f
    return 0, uy * f, uz * f
  end
  ...
  SDS.pressure = function(x,y,z)
    local dp_dt_torr_mm =     -- pressure gradient (torr/mm)
       vt_m_psec * (-8) * mu_pa_s * (d_mm/MM_M)^-2   -- Pa/m
       * TORR_PA / MM_M
    local rc = sqrt((y-y0_mm)^2 + (z-z0_mm)^2)
    local r = (rc - R0) / (0.5*d_mm)  -- (unitless, scaled to +-1 range)
    local theta = atan2(z-z0_mm, y-y0_mm) -- CCW with 0 deg at top
    if theta < 0 then theta = theta + 2 * math.pi end
    local tdist = theta * rc
    local p = (r>=-1 and r<=1) and p0_torr + tdist * dp_dt_torr_mm or 0
    --print('DEBUG',p,theta)
    assert(p >= 0, "pressure became negative")
    return p
  end
...
--[[
  This is similar to define_poiseuille_cylindrical_transverse
  but flow velocity is constant along radius and pressure is globally constant.
--]]
function M.define_cylindrical_transverse(t)
  t = begin_check(t)
  local SDS       = t.SDS
  local d_mm      = t.d_mm
  local y0_mm     = t.y0_mm
  ...
  SDS.velocity = function(x,y,z)
    local rc = sqrt((y-y0_mm)^2 + (z-z0_mm)^2)
    local r = (rc - R0) / (0.5*d_mm)  -- (unitless, scaled to +-1 range)
    local vt = (abs(r) <= 1) and vt_m_psec or 0  -- (m/s)
    local f = vt / rc
    local uy = -(z-z0_mm) * f
    local uz =  (y-y0_mm) * f
    return 0, uy * f, uz * f
  ...
  -- note: SDS.pressure not customized here.
  -- note: SDS.temperature not customized here.
end
