...
SIMION_INSTALL_DIR = pathlib.Path("/home/andres/projects/workshop_Digital_Twins/SIMION")

# Where to save your results (next to this script).
RESULTS_DB  = SIM_FOLDER / f"beamline_study_phase{PHASE}.db"
RESULTS_CSV = SIM_FOLDER / f"beamline_results_phase{PHASE}.csv"
STUDY_NAME  = f"minimalsim_phase{PHASE}"
...
FOCUS_REGION = {"x": (8.7, 15.0), "y": (70, 83), "z": (71.8180, 81.0)}
    ...
    # Electrodes to optimize in the first phase:
OPTIMIZE_PHASE1  = {
    3:  (0.0, 500.0),   # Einzel lens 1 (center)
}
...
FIXED_PHASE1 = {
    ...
    4:  0.0, 5: 0.0, 7: 0.0, 8: 0.0,    # Einzel outer rings (grounded)
    13: 0.0, 14: 0.0, 16: 0.0, 17: 0.0, # ground plates
    6:  0.0,   # Einzel lens 2 (center)
    9:  0.0,   # Quadrupole bender
    10: 0.0,   # Quadrupole bender
    11: 0.0,   # Quadrupole bender
    12: 0.0,   # Quadrupole bender
    15: 0.0,   # Voltage / deflection plate 1
    18: 0.0,   # Voltage / deflection plate 2
}
...
z_iob = f"Z:{IOB_FILE}"
FLY_COMMAND = f'wine simion.exe --nogui fly --retain-trajectories=0 --restore-potential=0 "{z_iob}"'
...
def apply_voltages(chosen: dict) -> None:
    ...
    """
    all_volt = {**FIXED, **chosen}
    settings = ",".join(f"{n}={v:.5f}" for n, v in sorted(all_volt.items()))

    z_pa0 = f"Z:{PA0_FILE}"
    
    command = f'wine simion.exe --nogui --noprompt fastadj "{z_pa0}" {settings}'
    run_simion(command)
...
def objective(trial: Trial) -> float:
    ...
    try:
        ...
        print(f"  [DEBUG] stdout preview: {simion_output[:300]!r}")

    except subprocess.CalledProcessError as e:
        print(f"  [DEBUG] SIMION failed: {e.output}")
        return BAD_SCORE
    ...
    if OBJECTIVE == "hits" and PHASE == 1:
        # Not only gives the number of hit, but also the mean distance of the hits (moving toward -x):
        W_HITS = 0.9
